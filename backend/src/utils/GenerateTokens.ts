import { IUserDocument } from "../models/users.model.js";
import { ApiError } from "./ApiError.js";

type AccessRefreshType = {
  refreshToken: string;
  accessToken: string;
};
...
export const generateAccessRefreshToken = async (
  ...
  try {
    ...
    await user.save({ validateBeforeSave: false }); //this will not run th validation on the schema struct and types just update it
